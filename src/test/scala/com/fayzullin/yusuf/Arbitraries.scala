...
import domain.{orders, arAssociations}
import domain.arAssociations._
import domain.arAssociations.ArAssociation
...
trait PetStoreArbitraries {
  ...
  implicit val association = Arbitrary[ArAssociation] {
    for {
      target <- arbitrary[String]
      response <- arbitrary[String]
      id <- Gen.option(Gen.posNum[Long])
    } yield arAssociations.ArAssociation(target, response, id)
  }
